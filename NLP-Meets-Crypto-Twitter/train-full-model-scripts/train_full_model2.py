...
def train_model():
	...
	df_sent = pd.read_csv('sentiment2_svm250.csv')
	df['Sentiment'] = df_sent['sentiment']/2
	...
	features_train = df.iloc[:304, :].values
	labels_train = labels[:304, :]
	features_test = df.iloc[304:len(df)-14-1, :].values
	labels_test = labels[304:len(df)-1, :]
	...
	for j in range(1):
		...
		history = regressor.fit(X_train, y_train, epochs = 200,
