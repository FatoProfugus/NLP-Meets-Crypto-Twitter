...
def train_model(embedding_size):
	wv = Word2Vec.load('2021_word2vec_lightcrypto' + str(embedding_size)+ '.model').wv

	print("starting embedding_size " + str(embedding_size))
	print("before gen_input")
	df = pd.read_csv('lightcrypto.csv')
	...
	df.to_csv('2021_lightcrypto_sentiment6_svm' + str(embedding_size) + '.csv')
	return accuracy
