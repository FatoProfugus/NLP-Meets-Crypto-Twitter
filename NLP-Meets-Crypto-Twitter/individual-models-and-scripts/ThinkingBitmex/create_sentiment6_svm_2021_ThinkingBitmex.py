...
def train_model(embedding_size):
	wv = Word2Vec.load('2021_word2vec_ThinkingBitmex' + str(embedding_size)+ '.model').wv

	print("starting embedding_size " + str(embedding_size))
	print("before gen_input")
	df = pd.read_csv('ThinkingBitmex.csv')
	print(df.iloc[366:366+304][:])
	X_train = gen_input(wv, df.iloc[366:366+304][:], embedding_size)
	X_test = gen_input(wv, df.iloc[366+304:len(df)-14][:], embedding_size)
	print('cobie X_train len ' + str(len(X_train)))
	print('X_test len ' + str(len(X_test)))
	print("after gen_input")

	print("before gen_labels")
	df = pd.read_csv('labels_6.csv')
	...
	df.to_csv('2021_ThinkingBitmex_sentiment6_svm' + str(embedding_size) + '.csv')
	return accuracy
